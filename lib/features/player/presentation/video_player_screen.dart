...
class _VideoPlayerScreenState extends ConsumerState<VideoPlayerScreen> {
  ...
  void _onPlayerStateChanged() {
    if (mounted) {
      ref.read(subtitleProvider.notifier).updatePosition(_playerService.state.position);
      setState(() {});
    }
  }
  ...
  Future<void> _loadSubtitleFromFile(String path) async {
    try {
      ...
      if (content != null) {
        ref.read(subtitleProvider.notifier).loadContent(content, fileName: fileName);
        ...
        return GestureDetector(
          ...
          onDoubleTapDown: (details) {
            ...
            if (x < screenWidth * 0.35) {
              ...
              _playerService.seek(newPos < Duration.zero ? Duration.zero : newPos);
              _showSeekHUD(-10);
            } else if (x > screenWidth * 0.65) {
              // Double tap right: Forward 10s
              final newPos = pState.position + const Duration(seconds: 10);
              _playerService.seek(newPos > pState.duration ? pState.duration : newPos);
              _showSeekHUD(10);
            } else {
          ...
          onVerticalDragUpdate: (details) {
            ...
            if (x < screenWidth * 0.5) {
              ...
              setState(() {
                _brightnessIndicator = ((_brightnessIndicator ?? 0.5) + delta).clamp(0.0, 1.0);
              });
  ...
  Widget _buildGestureIndicators() {
    if (_volumeIndicator == null && _brightnessIndicator == null && _seekDragTarget == null) {
        ...
        child: Column(
          ...
          children: [
            ...
            if (_seekDragTarget != null) ...[
              const Icon(Icons.fast_forward, color: AppColors.primaryLight, size: 36),
              const SizedBox(height: 8),
              Text(
          ...
          child: Row(
            children: [
              ...
              IconButton(
                ...
                icon: const Icon(Icons.palette_outlined, color: AppColors.primaryLight),
                    ...
                    child: SliderTheme(
                      ...
                      child: Slider(
                        value: pState.duration.inMilliseconds > 0
                            ? pState.position.inMilliseconds.clamp(0, pState.duration.inMilliseconds).toDouble()
                            : 0.0,
                ...
                children: [
                  ...
                  Row(
                    children: [
                      IconButton(
                        icon: Icon(
                          pState.isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled,
                          size: 38,
                          color: AppColors.primaryLight,
                        ),
                      ...
                      PopupMenuButton<double>(
                        ...
                        onSelected: (rate) => _playerService.setPlaybackRate(rate),
                        child: Container(
                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        tooltip: subState.isVisible ? 'إخفاء الترجمة' : 'إظهار الترجمة',
                        icon: Icon(
                          subState.isVisible ? Icons.closed_caption : Icons.closed_caption_disabled,
                          color: subState.isVisible ? AppColors.primaryLight : Colors.white38,
                        ),
                        onPressed: () {
                          ref.read(subtitleProvider.notifier).toggleVisibility();
                        },
                      ),
                    ],
                  ),
